...
| `--scrollbar █` | long switch/session/settings lists | Covered for app lists | `nativeListLinesWithScrollbar`; proportional multi-row thumb rendering in `projmuxpicker`; `TestNativeInteractiveUsesScrollbarForLongLists`; `TestListLinesWithScrollbarUsesProportionalThumb`; `TestListLinesWithScrollbarMovesThumbGradually` |
...
| selected multi-line marker | selected switch/session/notify cards | Covered for app multiline rows | native uses a compact single red `|` continuation gutter for selected metadata lines; `nativeContinuation`; `TestNativeInteractiveRendersSelectedMultilineContinuationMarker`; `TestInteractiveRowLinesUsesCompactSelectedMetaIndent` |
