...
| redraw flicker/top clipping | keyboard navigation in exact-height tmux popup | Partially covered | native redraws use synchronized updates plus row diffs after the first frame, skip unchanged frames, render frame diffs before sidebar focus commands, and frame rendering avoids trailing bottom-border CRLF; `TestNativeInteractiveWrapsRedrawsInSynchronizedUpdates`; `TestFrameUpdateRendererSkipsUnchangedFrame`; `TestRendererRenderFrameUsesCRLFRowsForRawTTY` |
