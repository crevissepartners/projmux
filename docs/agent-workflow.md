...
- `make test` also covers `projmux shell --saved|--layout <name>|--empty` explicit new-session selectors, saved/preset/empty candidate ordering for the deferred startup picker, and the existing-session guard that prevents shell startup replay from overwriting a live app session.
...
- `make test` also covers `projmux ai integrate claude` user-level Claude Code hook settings install/idempotence, dry-run preview, unmanaged projmux ingest command conflict refusal, managed-command removal, preservation of unrelated settings/hooks, and managed wiring for core plus extra Claude events.
- `make test` also covers `projmux ai ingest claude-hook` core and extra Claude Code hook ingest for event parsing, transcript fallback, permission summary formatting, UserPromptSubmit busy/no-queue behavior, Notification severity/text mapping, StopFailure/SubagentStop/TeammateIdle text/severity/metadata mapping, and hook-active pane marking.
