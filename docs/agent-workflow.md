...
- `make test` also covers project layout preset schema/discovery for `.projmux/layouts/*.toml`, supported interpolation validation, malformed-file list warnings, and user-facing `projmux layout list|show|save|remove` behavior including portable save paths, `--description`, `--fresh`, and `remove --force` safety.
