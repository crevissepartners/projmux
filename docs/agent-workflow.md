...
- `make test`: fast unit coverage for app-layer current, kill, pin, preview, session-popup, switch, tag, and tmux helper commands, preview select writes, popup render output after cycling, switch picker tag actions, switch/popup rendering, session identity, candidate discovery, config path derivation, popup preview read-models, and pure state rules including preview and tag stores.
