...
- `make test`: fast unit coverage for app-layer attach/current/kill/pin/preview/prune/sessions/session-popup commands, switch, tag, and tmux helper commands, preview select writes, popup render output after cycling, switch picker tag/pin/settings actions and loop behavior, switch default-root, pretty-path, preview-context, settings sentinel preview/menu flows including add-current-pin, interactive add-pin picker, and settings label/preview polish, fzf preview wiring, fzf baseline surface parity including prompt/footer/header fallback, preview metadata rendering, switch preview cycle bindings, sessions picker preview/cycle/open wiring, richer sessions row metadata rendering including window-count and stored-target badges, popup/switch preview summary formatting, popup sessions tmux entry helpers, switch/popup/session rendering, session identity, candidate discovery, config path derivation, popup preview read-models, and pure state rules including preview, tag, and lifecycle stores.
