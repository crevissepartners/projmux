...
- Picker focused unit coverage includes backend-neutral picker item/action mapping, fzf adapter output, native title-focused filtering, numeric selection, shared close actions, saved picker backend config, Settings Labs backend switching, environment override precedence, compact selected and unselected multi-line metadata gutters, proportional native scrollbar thumb rendering, and fixed split-preview list viewports with scrollbar tracks.
