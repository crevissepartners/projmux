...
plumbing, so its only entrypoint is the hidden `internal` namespace. Regenerate
managed config during upgrade instead of retaining a pre-namespace command.
...
| -------- | ------------- | ----------------------------------------- | ------------- |
| `notify`  | `status-format[0]` | `projmux internal focus --target <newest> --source status-bar --kind segment-click [--client <tty>]`, then ack on focus success | `prefix s n`  |
...
a live reply badge and the queue disagree, use `projmux get notifications --live`.
...
bind-key -T projmux-status u run-shell '#{q:projmux} internal statusbar click usage'
bind-key -T projmux-status r run-shell '#{q:projmux} internal statusbar usage-refresh'
bind-key -T projmux-status n run-shell '#{q:projmux} internal statusbar click notify'
bind-key -T projmux-status g run-shell '#{q:projmux} internal statusbar click git'
bind-key -T projmux-status p run-shell '#{q:projmux} internal statusbar click pwd'
bind-key -T projmux-status s run-shell '#{q:projmux} internal statusbar click session'
```
...
The status bar is generated by `projmux config render standalone` or
`projmux config render app`. To change a segment, edit the generator (it
