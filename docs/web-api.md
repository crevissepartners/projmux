...
| GET | `/api/v1/usage` | status bar usage HUD and popup | `{hud, rows, unsupported, lastSync, syncSource, error}` from the cache only; never collects. `hud` is what the bar draws under the web usage visibility (`web.toml ?? TUI files`; a `web.toml` that does not parse is reported in `error` and draws no `hud`), `rows` what its popup lists |
