...
- **Operator source.** A browser has no Pane, and `agent message send` needs a
  source Agent. Until projmux has a source kind for an operator, the client
  anchors on the target Agent itself and signs `messageRef` with the prefix
