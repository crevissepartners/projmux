...
| GET | `/api/v1/usage` | status bar usage HUD and popup | `{hud, rows, unsupported, lastSync, syncSource, error}` from the cache only; never collects. `hud` is what the bar draws under the web usage visibility (`web.toml ?? TUI files`; a `web.toml` that does not parse is reported in `error` and draws no `hud`; keys this build does not know are skipped), `rows` what its popup lists |
...
| GET | `/api/v1/web/settings` | the Settings a web page consumes: `{ai:{defaultMode, modes, providers, splitCwdFrom, splitCwdOrigin}, statusbar:{…parts, usageProviders}, locale, origins}`. Front settings (`ai.splitCwdFrom`, the status bar parts but `resources`, the usage leaves) read `web.toml ?? TUI/central file ?? default`; `origins` maps each front PATCH key to where its value came from: `web`, `tui` or `default`, and for `ai.splitCwdFrom` `web`, `global` or `default` (also `splitCwdOrigin`). A `web.toml` that does not parse answers `409 refused` with `<path>:<line>: …` (keys this build does not know are skipped, not refused) and `details:{file, line}`; see [configuration](configuration.md#web-settings-webtoml) |
| PATCH | `/api/v1/web/settings` | `{key, value}`: changes one of them. Keys: `ai.defaultMode`, `ai.provider.<id>`, `ai.splitCwdFrom`, `statusbar.<notifications\|usage\|project\|working-directory\|git\|resources\|clock>`, `statusbar.usage.<provider>[.<window>]`, `locale`. Front keys (`ai.splitCwdFrom`, `statusbar.*` but `resources`) are written to `web.toml` only, touch no TUI file and reload nothing; a `web.toml` that does not parse is refused like the GET and left as it is, and keys this build does not know are written back. `ai.defaultMode`, `ai.provider.<id>`, `locale` and `statusbar.resources` are saved through the function the terminal Settings uses; `statusbar.resources` regenerates the tmux config and reloads the app server through its own logical socket, never an inherited `TMUX`, and a save that lands but does not reload answers `refused` |
| GET | `/api/v1/web/statusbar` | which web status bar parts are on: `{notifications, usage, project, workingDirectory, git, resources, clock}`, each `web.toml ?? TUI file ?? default`; `resources` is the central `live-resources`. A `web.toml` that does not parse answers as the settings GET does |
