...
//
// Hidden is not removal. The internal plumbing Phase moved every plumbing route
// under the hidden `internal` namespace and hid the current spellings from the
// primary listing; all of them stay dispatchable, because a tmux server that is
// already running holds config generated by a previously installed binary and
// keeps invoking the current spellings.
var routes = []Route{
	{
	...
	{
		// Internal plumbing compatibility alias. The canonical spelling is
		// `internal preview`; this top-level route stays dispatchable so a tmux
		// server whose config was generated by a previously installed binary
		// keeps working, but it is out of the primary help listing.
		Name:        "preview",
	...
	{
		// Internal plumbing compatibility alias; canonical spelling is
		// `internal status` (except `status usage`, whose canonical spelling is
		// the Agent-domain `agent usage`).
		Name:        "status",
	...
	// from the primary help listing. Both are compatibility aliases now; the
	// canonical spellings live under the hidden `internal` namespace.
	{
		Name:        "key-broker",
		Summary:     "Forward captured physical key chords into the tmux root table",
		Disposition: DispositionInternal,
