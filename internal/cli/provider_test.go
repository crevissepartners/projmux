...
func TestProviderCreateShortcutsAreOptInAndCollideWithNoKindOrVerb(t *testing.T) {
	...
	if !reflect.DeepEqual(kinds, []string{"project", "window", "pane", "agent", "notification", "snapshot"}) {
		t.Fatalf("create kind children = %v, want [project window pane agent notification snapshot]", kinds)
	}
...
func TestAgentDomainNamespaceOwnsTheAgentWorkflowSpellings(t *testing.T) {
	...
	if want := []string{"status", "topic", "resume", "turn", "approval", "review", "integrate", "usage"}; !reflect.DeepEqual(children, want) {
