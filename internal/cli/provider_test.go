...
func TestProviderCreateShortcutsAreOptInAndCollideWithNoKindOrVerb(t *testing.T) {
	...
	if !reflect.DeepEqual(kinds, []string{"project", "window", "pane", "agent", "notification"}) {
		t.Fatalf("create kind children = %v, want [project window pane agent notification]", kinds)
	}
