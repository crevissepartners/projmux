...
func TestExperimentalInitializeGatesAdditionalRootsOnTheWire(t *testing.T) {
	t.Run("experimental initialize carries roots to the wire", func(t *testing.T) {
		...
		if _, err := client.StartThread(ctx, "/work/project", []string{"/work/extra", " ", "/work/second"}, "", ThreadPolicy{}); err != nil {
			t.Fatal(err)
		}
		frames := collect()
		if len(frames) < 3 {
	...
	t.Run("plain initialize negotiates nothing and refuses roots", func(t *testing.T) {
		...
		for name, call := range map[string]func() error{
			methodThreadStart: func() error {
				_, err := client.StartThread(ctx, "/work/project", []string{"/work/extra"}, "", ThreadPolicy{})
				return err
			},
			methodThreadResume: func() error {
				_, err := client.ResumeThread(ctx, "thread-exact", "/work/project", []string{"/work/extra"}, ThreadPolicy{})
				return err
			},
		} {
			err := call()
		...
		if _, err := client.StartThread(ctx, "/work/project", []string{" "}, "", ThreadPolicy{}); err != nil {
			t.Fatal(err)
		}
		frames := collect()
		if len(frames) != 3 {
