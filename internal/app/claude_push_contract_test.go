...
func (p *barrierClaudePoster) Post(string) (claudeProviderPostOutcome, error) {
	p.calls++
	close(p.started)
	<-p.release
...
func TestClaudeDialogueMultiplePendingAbandonsAllButLaterExactCandidate(t *testing.T) {
	...
	if original == nil || original.MessageRef != "message-later" || reason != "" {
		t.Fatalf("later original=%+v reason=%q", original, reason)
	}
	hub.finishReply(original.MessageRef, "reply-later", true)
