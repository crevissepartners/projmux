...
func (p *qualificationBarrierPoster) Post(string) (claudeProviderPostOutcome, error) {
	p.mu.Lock()
	p.calls++
	p.mu.Unlock()
	close(p.started)
...
func (p *qualificationPosterRecorder) Post(content string) (claudeProviderPostOutcome, error) {
	p.calls++
	p.content = content
