...
func TestSettingsSaveRefusesStandaloneLiveReloadWithoutTmuxWrites(t *testing.T) {
	t.Parallel()
	...
	if err == nil || !strings.Contains(err.Error(), "refuses an operator-owned standalone tmux server") {
		t.Fatalf("standalone Settings reload error = %v", err)
	}
	if got := readFile(t, filepath.Join(home, ".config", "projmux", "keymap.toml")); !strings.Contains(got, "[bindings.ProjectSidebarToggle]\nkeys = [\"M-a\"]\n") {
		t.Fatalf("durable standalone keymap save = %q", got)
	}
	if got := readFile(t, filepath.Join(home, ".config", "projmux", "tmux.conf")); !strings.Contains(got, "Generated by projmux") {
		t.Fatalf("durable standalone generated config = %q", got)
	}
	for _, call := range server.calls {
		argv := tmuxCommandArgv(call)
		if slices.Contains(argv, "source-file") || slices.Contains(argv, "unbind-key") ||
			slices.Contains(argv, "set-option") || slices.Contains(argv, "set-hook") {
			t.Fatalf("standalone Settings refusal reached a tmux write: %#v", call)
		}
	}
}
