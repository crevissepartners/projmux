...
func TestManagedProjectStopLowersOnlyTheExactStoredSessionProjection(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "matching live projection keeps its name and socketPath",
			stored: &coremetadata.SessionProjection{Name: "alpha", Live: true, SocketPath: "/tmp/projmux-stop"},
			want:   &coremetadata.SessionProjection{Name: "alpha", Live: false, SocketPath: "/tmp/projmux-stop"}},
