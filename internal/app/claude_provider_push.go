...
type claudeProviderPoster interface {
	Post(string) (claudeProviderPostOutcome, error)
}
...
func (p *liveClaudeProviderPoster) Post(content string) (claudeProviderPostOutcome, error) {
	frame, err := buildClaudeProviderPushFrame(p.token, content)
	if err != nil || p.current == nil || !p.current() {
	...
	if err != nil || peer != p.process || !p.current() {
		return claudeProviderPostOutcome{}, errors.New("Claude provider push refused")
	}
	identity, err = inspectClaudeSocket(p.socket)
	if err != nil || identity != p.socketIdentity || !p.current() {
		return claudeProviderPostOutcome{}, errors.New("Claude provider push refused")
	}
