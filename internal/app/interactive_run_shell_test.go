...
func TestInteractiveRunShellGuardCoversEveryLedgeredRouteAndNothingElse(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "window delete", argv: []string{"internal", "tmux", "window-delete", "--client", "/dev/pts/3", "--anchor", "%4"},
			guarded: true, want: interactiveRouteWindowDelete, client: "/dev/pts/3"},
		{name: "managed delete confirm", argv: []string{"internal", "tmux", "delete-confirm", "--client", "/dev/pts/3", "--anchor", "%4", "pane"},
			guarded: true, want: interactiveRouteDeleteConfirm, client: "/dev/pts/3"},
