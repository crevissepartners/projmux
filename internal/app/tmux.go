...
func newTmuxCommand(recorders ...*diagnostics.LifecycleRecorder) *tmuxCommand {
	...
	cmd.launchChoose = func(anchorPaneID, client string) launchChoice {
		if cmd.ai == nil {
			return launchChoice{}
		}
...
func (c *tmuxCommand) runWindowCreateIntent(args []string, stdout, stderr io.Writer) error {
	...
	pressing := strings.TrimSpace(*client)
	// Ask first. The answer is decided before anything is created, on the Pane
	// the key was pressed in: a picker that is cancelled leaves nothing behind,
	// and one that cannot be asked says so without a half-made Window.
	var choice launchChoice
	if c.launchChoose != nil {
		choice = c.launchChoose(*anchor, pressing)
	}
	switch {
	case choice.problem != "":
...
func buildPopupToggleWithStyle(mode tmuxPopupToggleMode, binaryPath, marker string, ctx tmuxPopupContext, lookupEnv func(string) string, popupBodyStyle string) (string, inttmux.PopupOptions, error) {
	...
	case "ai-split-picker-right", "ai-split-picker-down":
		...
		addSplitProducerEnv(env, &options, mode, ctx)
