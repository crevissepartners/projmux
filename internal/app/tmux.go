...
func (c *tmuxCommand) Run(args []string, stdout, stderr io.Writer) error {
	...
	case "status-click":
		return c.runStatusClick(fs.Args()[1:], stderr)
	case "print-config":
...
func (c *tmuxCommand) runStatusClick(args []string, stderr io.Writer) error {
	fs := flag.NewFlagSet("tmux status-click", flag.ContinueOnError)
	fs.SetOutput(stderr)
	client := fs.String("client", "", "tmux client key used to scope popup markers")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if fs.NArg() != 1 {
		printTmuxUsage(stderr)
		return errors.New("tmux status-click requires exactly 1 argument: <range>")
	}

	click := parseStatusClickRange(fs.Arg(0))
	if click == "" {
		return nil
	}
	switch click {
	case "projmux-session":
		return c.runPopupToggle(statusClickPopupArgs(*client, "session-popup"), stderr)
	case "projmux-project", "projmux-git":
		return c.runPopupToggle(statusClickPopupArgs(*client, "sessionizer-sidebar"), stderr)
	case "projmux-kube":
		return c.runKubeStatusPopup(stderr)
	default:
		return nil
	}
}

func statusClickPopupArgs(client, mode string) []string {
	args := []string{}
...
func parseStatusClickRange(raw string) string {
...
func (c *tmuxCommand) runKubeStatusPopup(stderr io.Writer) error {
	if c.runner == nil {
		return errors.New("configure tmux runner: tmux runner is not configured")
	}
	if c.executable == nil {
	...
	session := strings.TrimSpace(popupCtx.OriginSession)
	command := "printf " + tmuxShellQuote("Kubernetes status\n\n") + "; " + tmuxShellQuote(binaryPath) + " status kube"
	if session != "" {
		command += " " + tmuxShellQuote(session)
	}
	command += "; printf " + tmuxShellQuote("\n\nPress Enter to close") + "; read _"

	displayArgs, err := inttmux.BuildDisplayPopupArgs(command, inttmux.PopupOptions{
		Target:        popupCtx.OriginPane,
		Width:         popupSize(popupCtx.ClientWidth, 45, 72),
		Height:        popupSize(popupCtx.ClientHeight, 25, 10),
		Title:         "projmux kube",
	...
	if _, err := c.runner.Run(ctx, "tmux", displayArgs...); err != nil {
		...
		return fmt.Errorf("display kube status popup: %w", err)
	}
	_ = stderr
	return nil
}
...
func printTmuxUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  projmux tmux status-click [--client <key>] <range>")
...
func tmuxStandaloneConfig(binaryPath string) string {
	...
	statusRight := strings.Join([]string{
		tmuxStatusRange("projmux-project", "#[fg=colour242]#{=/28/...:pane_current_path}"),
		"#[fg=colour239]  ",
		tmuxStatusRange("projmux-kube", "#("+bin+" status kube)"),
		tmuxStatusRange("projmux-git", "#("+bin+" status git)"),
		"  %Y-%m-%d %H:%M #[bold,fg=colour16,bg=colour45] projmux #[default]",
	}, "")
	lines := []string{
		...
		"set -g status-right " + tmuxConfigQuote(statusRight),
		"unbind-key -q -n M-1",
		"unbind-key -q -n M-2",
		"unbind-key -q -n M-3",
		"unbind-key -q -n M-4",
		...
		"bind-key -n MouseDown1StatusLeft run-shell -b " + tmuxConfigQuote(tmuxStatusClickCommand(bin)),
		"bind-key -n MouseDown1StatusRight run-shell -b " + tmuxConfigQuote(tmuxStatusClickCommand(bin)),
...
func tmuxAppConfig(binaryPath string) string {
	...
	statusLeft := tmuxStatusRange("projmux-session", "#[bold,fg=colour231,bg=colour90] #{s|^[^-]*-||:session_name} #[default]")
	statusRight := strings.Join([]string{
		tmuxStatusRange("projmux-project", "#[fg=colour242]#{=/28/...:pane_current_path}"),
		"#[fg=colour239]  ",
		tmuxStatusRange("projmux-kube", "#("+bin+" status kube)"),
	...
	lines = append(lines,
		"set -g status-left "+tmuxConfigQuote(statusLeft),
		"set -g status-right "+tmuxConfigQuote(statusRight),
	)
	return strings.Join(lines, "\n") + "\n"
}

func tmuxStatusRange(id, content string) string {
	return "#[range=user|" + id + "]" + content + "#[norange]"
}

func tmuxStatusClickCommand(bin string) string {
	return bin + " tmux status-click --client " + tmuxShellQuote("#{client_tty}") + " " + tmuxShellQuote("#{mouse_status_range}")
}
