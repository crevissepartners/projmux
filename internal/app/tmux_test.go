...
func TestTmuxPrintConfigUsesStandaloneBindings(t *testing.T) {
	...
	for _, want := range []string{
		...
		"range=user|usage",
		"#[align=left range=user|usage]#('/tmp/proj mux/bin/projmux' status usage --max-width 120)#[norange] #[bold,fg=colour16,bg=colour149]#[range=user|sessionstate] state #[norange]#[default]#[align=right range=user|notify]",
...
func TestTmuxPrintAppConfigUsesIsolatedAppSettings(t *testing.T) {
	...
	for _, want := range []string{
		...
		"range=user|sessionstate",
		"range=user|usage",
		"#[align=left range=user|usage]#('/tmp/projmux' status usage --max-width 120)#[norange] #[bold,fg=colour16,bg=colour149]#[range=user|sessionstate] state #[norange]#[default]#[align=right range=user|notify]",
