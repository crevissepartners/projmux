...
func TestTmuxPrintConfigUsesStandaloneBindings(t *testing.T) {
	...
	for _, want := range []string{
		...
		"sleep 0.05; '/tmp/proj mux/bin/projmux' internal tmux rebalance-panes >/dev/null 2>&1 || true; " +
			"'/tmp/proj mux/bin/projmux' internal tmux converge --socket-path '#{socket_path}' --session '#{session_id}' --reason runtime-exited >/dev/null 2>&1 || true",
		...
		"#[align=left range=user|notify]#('/tmp/proj mux/bin/projmux' internal status notify --max-width #{?#{e|<:#{client_width},40},#{e|/:#{client_width},2},#{?#{e|<:#{client_width},160},20,#{?#{e|<:#{client_width},220},#{e|-:#{client_width},140},80}}})#[norange]#[align=right range=user|usage]#('/tmp/proj mux/bin/projmux' internal status usage --max-width #{e|-:#{client_width},#{?#{e|<:#{client_width},40},#{e|/:#{client_width},2},#{?#{e|<:#{client_width},160},20,#{?#{e|<:#{client_width},220},#{e|-:#{client_width},140},80}}}})#[norange]",
		"align=left",
		"align=right",
