...
func TestTmuxPrintConfigUsesStandaloneBindings(t *testing.T) {
	...
	for _, want := range []string{
		...
		"#[align=left range=user|notify]#('/tmp/proj mux/bin/projmux' internal status notify --max-width #{?#{e|<:#{client_width},160},#{e|/:#{client_width},2},80})#[norange]#[align=right range=user|usage]#('/tmp/proj mux/bin/projmux' internal status usage --max-width #{e|-:#{client_width},#{?#{e|<:#{client_width},160},#{e|/:#{client_width},2},80}})#[norange]",
		"align=left",
		"align=right",
...
func TestTmuxPrintAppConfigUsesIsolatedAppSettings(t *testing.T) {
	...
	for _, want := range []string{
		...
		"set -g status-format[0]",
		"set -g status-format[1]",
		"#[align=left range=user|notify]#('/tmp/projmux' internal status notify --max-width #{?#{e|<:#{client_width},160},#{e|/:#{client_width},2},80})#[norange]#[align=right range=user|usage]#('/tmp/projmux' internal status usage --max-width #{e|-:#{client_width},#{?#{e|<:#{client_width},160},#{e|/:#{client_width},2},80}})#[norange]",
