...
func typedExemptSurface(id, producer, handler, class, roots, owner string, verb runtimeMutationVerb, reason, effect string) runtimeMutationSurface {
	row := exemptSurface(id, producer, handler, class, roots, owner, reason, effect)
	row.PlanVerb = string(verb)
	return row
}
...
var runtimeMutationSurfaces = []runtimeMutationSurface{
	...
	plannedSurface("project.bootstrap-route-marker", "fresh Project materializer bootstrap", "materializer.writeCreatedProjectRouteMarker", "managed route identity", "Project", "exact freshly created $/@/% and operation lease", mutationWriteRouteMarker, "generated app config ownership, exact physical socket, created tuple and lease", "logical route marker equals the declared Project invocation route"),
	...
	// The two extra writes `create window --provider` performs inside the very
	// same transaction as public.create-window. They are listed apart from it
	// because a surface row binds one producer to one plan verb, and this
	// producer issues three.
	plannedSurface("public.create-window.provider-agent", "public create Window --provider", "canonical createWindow openWindowAgentPane", "managed topology", "Project|ControlSession", "the initial Pane this create just materialized", mutationCreatePane, "socket, root UID, Window UID and anchor containment", "one owned Agent Pane beside the Window's initial Pane"),
	plannedSurface("public.create-window.provider-retire-shell", "public create Window --provider", "canonical createWindow retireWindowInitialShell", "managed lifecycle", "Project|ControlSession", "the initial Pane this create allocated and mirrored", mutationKillOwned, "Registry row removed first, then the same mirrored operation-owned uid re-proved immediately before the write", "only the initial shell Pane this create made is absent; the Agent Pane remains as the Window's anchor"),
	...
	catalogSurface(plannedSurface("catalog.window.rename", "generated catalog rename-window (legacy alias)", "internal tmux window-rename", "managed presentation identity", "Project|ControlSession", "exact anchor Pane owner chain", mutationRenameWindow, "exact origin re-proved under the Registry lock of the shared rename owner, then socket, Window UID and containment", "Registry name and stable-name mirror change through the public rename Window owner, then the display name; UID and topology remain"), "rename-window"),
	// The two managed tmux close keys. The generated binding branches on the
	// identity mirror: only the mirrored branch reaches projmux, and it reaches
	// the same canonical delete the Pane menu Kill and `delete window` use. The
	// mirror-absent branch is tmux's own stock confirm-before binding, which
	// tmux runs; projmux issues no raw kill for either branch.
	catalogSurface(plannedSurface("catalog.pane.delete", "generated catalog prefix x on a mirrored Pane (legacy alias delete-pane)", "internal tmux delete-confirm pane -> internal tmux pane-menu kill -> canonical delete Pane", "managed lifecycle", "Project|ControlSession", "exact anchor Pane mirrored UID", mutationKillPane, "exact-client confirmation, then batch exact socket/UID/containment before commit; a mirror-absent Pane keeps tmux's stock binding and projmux issues nothing", "exact Pane and its owning Agent absent after durable result flush"), "delete-pane"),
	catalogSurface(plannedSurface("catalog.window.delete", "generated catalog prefix & on a mirrored Window (legacy alias delete-window)", "internal tmux delete-confirm window -> internal tmux window-delete -> tmuxWindowDeleteRuntime", "managed lifecycle", "Project|ControlSession", "exact anchor Pane owner Window UID", mutationKillWindow, "exact-client confirmation, then all exact targets guarded before first write; a mirror-absent Window keeps tmux's stock binding and projmux issues nothing", "selected owned Window with its Panes and Agents absent; a last Window leaves a zero-Window Project"), "delete-window"),
	// The generated Pane menus: MouseDown3Pane, and the app config's `prefix >`
	// and M-MouseDown3Pane that replace tmux's stock Pane menus. One row per
	// item; an item every menu carries maps to the same row in each of them.
	plannedSurface("pane-menu.split-right", "generated Pane menu Horizontal Split (MouseDown3Pane, prefix >, M-MouseDown3Pane)", "internal tmux pane-menu split-right", "managed topology", "Project|ControlSession", "exact menu-target Pane", mutationCreatePane, "socket, root UID and anchor containment", "one owned right Pane"),
	plannedSurface("pane-menu.split-down", "generated Pane menu Vertical Split (MouseDown3Pane, prefix >, M-MouseDown3Pane)", "internal tmux pane-menu split-down", "managed topology", "Project|ControlSession", "exact menu-target Pane", mutationCreatePane, "socket, root UID and anchor containment", "one owned down Pane"),
	plannedSurface("pane-menu.kill", "generated Pane menu Kill on a mirrored Pane (MouseDown3Pane, prefix >, M-MouseDown3Pane)", "internal tmux pane-menu kill -> canonical delete Pane", "managed lifecycle", "Project|ControlSession", "exact menu-target Pane mirrored UID", mutationKillPane, "menu selection is the confirmation, then batch exact socket/UID/containment before commit; a mirror-absent Pane runs tmux's stock kill-pane menu item, which tmux executes, and projmux issues nothing", "exact Pane absent after durable result flush"),
	// The app config's `prefix <`, MouseDown3Status and M-MouseDown3Status
	// replace tmux's stock Window menus; the three carry the same items.
	plannedSurface("window-menu.kill", "generated Window menu Kill on a mirrored Window (prefix <, MouseDown3Status, M-MouseDown3Status)", "internal tmux window-delete -> tmuxWindowDeleteRuntime", "managed lifecycle", "Project|ControlSession", "exact menu-target Pane owner Window UID", mutationKillWindow, "menu selection is the confirmation, then all exact targets guarded before first write; a mirror-absent Window runs tmux's stock kill-window menu item, which tmux executes, and projmux issues nothing", "selected owned Window with its Panes and Agents absent; a last Window leaves a zero-Window Project"),
	plannedSurface("window-menu.rename", "generated Window menu Rename (prefix <, MouseDown3Status, M-MouseDown3Status)", "internal tmux window-rename", "managed presentation identity", "Project|ControlSession", "exact menu-target Pane owner chain", mutationRenameWindow, "exact origin re-proved under the Registry lock of the shared rename owner, then socket, Window UID and containment", "Registry name and stable-name mirror change through the public rename Window owner, then the display name; UID and topology remain"),
	plannedSurface("window-menu.create", "generated Window menu New At End (prefix <, MouseDown3Status, M-MouseDown3Status)", "internal tmux window-create", "managed topology", "Project|ControlSession", "exact menu-target Pane owner chain", mutationCreateWindow, "socket, root UID, session and anchor containment", "one owned Window and primary Pane"),
	...
	catalogSurface(plannedSurface("catalog.project-sidebar.runtime.stop", "generated Project sidebar Ctrl-X (legacy alias Sidebar:KillSession)", "executeManagedRuntimeStop", "managed lifecycle", "Project", "resolved Project UID", mutationStopManagedSession, "retained-window Stop table cell plus exact app route and reattributed session ID/UID", "exact Project runtime session absent; Project UID and desired Window/Pane topology unchanged"), "Sidebar:KillSession"),
	catalogSurface(plannedSurface("catalog.session-picker.runtime.stop", "generated Session picker Ctrl-X (legacy alias SessionPopup:KillSession)", "executeManagedRuntimeStop", "managed lifecycle", "Project", "resolved Project UID", mutationStopManagedSession, "retained-window Stop table cell; unknown, unmatched, and ControlSession attribution refuses with zero kill", "exact Project runtime session absent; Project UID and desired Window/Pane topology unchanged"), "SessionPopup:KillSession"),
	...
	plannedSurface("controller.identity", "generated lifecycle trigger, explicit reconcile, and Project-open/materialize convergence", "resource controller plan", "managed identity", "Project|ControlSession", "exact -S plus app/standalone PID generation and resourcegraph handle", mutationWriteIdentity, "printable route authority, controller attribution, and containment", "identity mirrors only; lifecycle verbs refused"),
	plannedSurface("controller.option", "generated lifecycle trigger, explicit reconcile, and Project-open/materialize convergence", "resource controller plan", "managed closed name/path/automatic-rename and L8 routing-index projection", "Project", "exact -S plus app/standalone PID generation and resourcegraph handle", mutationWriteOption, "printable route authority, kind-exact UID attribution, containment, exact option before-value, and closed field/scope argv grammar", "only Project name/path, Window name/automatic-rename, Pane name, and L8 conversation-index cleanup converge; ControlSession role belongs to converge-control-identity"),
	plannedSurface("app.route-marker", "tmux config apply", "typed config-apply route marker write", "managed route identity", "all app-owned", "exact invocation socket", mutationWriteRouteMarker, "source-file success and app ownership", "logical route marker equals invocation route"),
	...
	typedExemptSurface("agent.presentation", "Agent topic/interaction and generation projection", "typed controller and provider lifecycle presentation adapters", "presentation", "Project|ControlSession", "exact Agent Pane UID, activation, and containment", mutationWritePresentationOption, "closed topic/manual-topic/state/badge/attention field grammar; generation-aware writes require exact endpoint generation, broker runtime, connection epoch, binding epoch, durable owner, and target runtime; UID/topology unchanged", "Agent presentation changes through guarded typed or exact native provider projection"),
	exemptSurface("codex.native-lifecycle-authority", "Codex native observer authority projection", "Codex lifecycle sink and observer startup", "operation metadata", "Agent Pane", "exact Registry Agent/Pane activation and runtime Pane UID", "authority, epoch, and bounded reason options only; UID/topology unchanged", "native observer authority provenance converges without lifecycle or topology mutation"),
	...
	catalogSurface(exemptSurface("catalog.pane.rename", "generated catalog Pane rename", "internal tmux pane-rename", "presentation identity", "Project|ControlSession", "exact anchor Pane mirrored UID and owner chain", "Registry rename through the public rename Pane owner with the exact origin re-proved under its lock; Pane UID and topology unchanged", "Pane Registry name and label projection change"), "rename-pane-label"),
	exemptSurface("internal.rename-pane-label", "internal tmux rename-pane helper (no generated binding)", "tmux presentation option", "presentation", "Project|ControlSession", "explicit tmux Pane target", "label-only writer outside the Registry; Pane UID, Registry name and topology unchanged", "Pane label option changes"),
	...
	exemptSurface("pane.rebalance", "explicit internal rebalance and generated Pane-exit hooks", "runRebalancePanes -> select-layout -E", "presentation", "all runtime classes", "every observed multi-Pane Window on the invoking tmux server", "all-window inventory; best-effort layout-only writes; topology and identity unchanged", "select-layout -E is attempted for every observed multi-Pane Window"),
	...
	exemptSurface("pane-menu.swap-marked", "generated Pane menu Swap Marked (prefix >, M-MouseDown3Pane)", "tmux stock swap-pane", "presentation", "Project|ControlSession", "menu-target Pane and the marked Pane", "display-order only; Pane UIDs, owner Window and topology unchanged", "Pane display order changes"),
	...
	exemptSurface("pane-menu.go-to-top", "generated Pane menu Go To Top (prefix >, M-MouseDown3Pane)", "tmux stock copy-mode", "copy-mode navigation", "all", "menu-target Pane in copy or view mode", "copy-mode cursor only", "copy-mode history top shown"),
	exemptSurface("pane-menu.go-to-bottom", "generated Pane menu Go To Bottom (prefix >, M-MouseDown3Pane)", "tmux stock copy-mode", "copy-mode navigation", "all", "menu-target Pane in copy or view mode", "copy-mode cursor only", "copy-mode history bottom shown"),
	exemptSurface("pane-menu.search-word", "generated Pane menu Search For word (prefix >, M-MouseDown3Pane)", "tmux stock copy-mode", "copy-mode navigation", "all", "mouse word under the menu-target Pane", "copy-mode search only", "copy-mode searches backward for the word"),
	exemptSurface("pane-menu.type-word", "generated Pane menu Type word (prefix >, M-MouseDown3Pane)", "tmux stock send-keys -l", "input forwarding", "all", "mouse word under the menu-target Pane", "literal input to the Pane's program only", "the word is typed into the Pane"),
	exemptSurface("pane-menu.copy-word", "generated Pane menu Copy word (prefix >, M-MouseDown3Pane)", "tmux stock set-buffer", "paste buffer", "all", "mouse word under the menu-target Pane", "paste buffer only", "the word is copied to a paste buffer"),
	exemptSurface("pane-menu.copy-line", "generated Pane menu Copy Line (prefix >, M-MouseDown3Pane)", "tmux stock set-buffer", "paste buffer", "all", "mouse line under the menu-target Pane", "paste buffer only", "the line is copied to a paste buffer"),
	exemptSurface("pane-menu.type-hyperlink", "generated Pane menu Type hyperlink (prefix >, M-MouseDown3Pane)", "tmux stock send-keys -l", "input forwarding", "all", "mouse hyperlink under the menu-target Pane", "literal input to the Pane's program only", "the hyperlink is typed into the Pane"),
	exemptSurface("pane-menu.copy-hyperlink", "generated Pane menu Copy hyperlink (prefix >, M-MouseDown3Pane)", "tmux stock set-buffer", "paste buffer", "all", "mouse hyperlink under the menu-target Pane", "paste buffer only", "the hyperlink is copied to a paste buffer"),
	// swap-window is a closed topology verb admitted only as these exact stock
	// Window menu presentation items: it reorders Window indexes and leaves
	// every Window UID, owner Project and Pane unchanged.
	exemptSurface("window-menu.swap-left", "generated Window menu Swap Left (prefix <, MouseDown3Status, M-MouseDown3Status)", "tmux stock swap-window -t :-1", "presentation", "Project|ControlSession", "menu-target Window and its left neighbour", "display-order only; Window UIDs, owner Project and topology unchanged", "Window display order changes"),
	exemptSurface("window-menu.swap-right", "generated Window menu Swap Right (prefix <, MouseDown3Status, M-MouseDown3Status)", "tmux stock swap-window -t :+1", "presentation", "Project|ControlSession", "menu-target Window and its right neighbour", "display-order only; Window UIDs, owner Project and topology unchanged", "Window display order changes"),
	exemptSurface("window-menu.swap-marked", "generated Window menu Swap Marked (prefix <, MouseDown3Status, M-MouseDown3Status)", "tmux stock swap-window", "presentation", "Project|ControlSession", "menu-target Window and the marked Pane's Window", "display-order only; Window UIDs, owner Project and topology unchanged", "Window display order changes"),
	exemptSurface("window-menu.mark", "generated Window menu Mark/Unmark (prefix <, MouseDown3Status, M-MouseDown3Status)", "tmux presentation", "presentation", "Project|ControlSession", "menu-target Window's active Pane", "mark-only", "Pane mark toggles"),
	...
	exemptSurface("trigger.after-kill-pane", "generated after-kill hook controller half", "observation trigger into controller.identity", "observation trigger", "all", "event exact socket", "controller command half is observation-only; the preceding rebalance write is classified separately", "controller.identity observes and may execute its guarded identity plan"),
	exemptSurface("trigger.after-kill-pane.rebalance", "generated after-kill hook layout half", "internal tmux rebalance-panes -> runRebalancePanes", "presentation trigger", "all runtime classes", "every observed multi-Pane Window on the event tmux server", "best-effort select-layout -E only; topology and identity unchanged", "select-layout -E is attempted for every observed multi-Pane Window before controller observation"),
	exemptSurface("trigger.pane-exited", "generated pane-exited hook controller half", "observation trigger into controller.identity", "observation trigger", "all", "event exact Pane and socket", "controller command half is observation-only; the preceding rebalance write is classified separately", "controller.identity observes and may execute its guarded identity plan"),
	exemptSurface("trigger.pane-exited.rebalance", "generated pane-exited hook layout half", "internal tmux rebalance-panes -> runRebalancePanes", "presentation trigger", "all runtime classes", "every observed multi-Pane Window on the event tmux server", "best-effort select-layout -E only; topology and identity unchanged", "select-layout -E is attempted for every observed multi-Pane Window before exact Pane controller observation"),
	exemptSurface("trigger.pane-died", "generated retained-pane death hook controller half", "observation trigger into controller.identity", "observation trigger", "managed Agent", "event exact retained Pane and socket", "controller command half is observation-only; pane_dead and canonical receipt decide authority", "controller.identity observes the retained dead Agent Pane"),
	exemptSurface("trigger.pane-died.rebalance", "generated retained-pane death hook layout half", "internal tmux rebalance-panes -> runRebalancePanes", "presentation trigger", "managed Agent", "every observed multi-Pane Window on the event tmux server", "best-effort select-layout -E only; topology and identity unchanged", "select-layout -E is attempted before exact retained Pane observation"),
	...
	plannedSurface("resource.rename-window", "public resource rename Window", "renameRuntimeWindow", "managed stable-name projection", "Project|ControlSession", "Registry Window UID and exact runtime Window handle", mutationWriteStableName, "exact route PID generation, root UID, Window UID and containment", "Window stable-name option changes; display name, UID, and topology remain"),
