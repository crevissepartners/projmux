...
var runtimeMutationSurfaces = []runtimeMutationSurface{
	...
	plannedSurface("window-menu.rename", "generated Window menu Rename (prefix <, MouseDown3Status, M-MouseDown3Status)", "internal tmux window-rename", "managed presentation identity", "Project|ControlSession", "exact menu-target Pane owner chain", mutationRenameWindow, "socket, Window UID and containment", "display name changes; stable metadata identity remains"),
	...
	plannedSurface("codex.generation-handover-fence", "agent app-server handover apply/resume", "codexHandoverEffects", "managed provider binding fence", "Agent Pane", "journaled exact Agent/Pane/runtime/generation/thread tuple", mutationCodexHandoverFence, "app-owned server generation and exact live mirrored Agent/Pane identity", "old native binding authority is absent before old endpoint stop"),
	plannedSurface("codex.generation-handover-restore", "agent app-server handover abort", "codexHandoverEffects", "managed provider binding restore", "Agent Pane", "journaled exact pre-stop Agent/Pane/runtime/generation/thread tuple", mutationCodexHandoverRestore, "pre-stop abort plus app-owned server generation and exact live mirrored identity", "old native binding authority is restored only before stop"),
	plannedSurface("codex.generation-handover-relaunch", "agent app-server handover apply/resume", "codexHandoverEffects", "managed same-Pane provider relaunch", "Agent Pane", "journaled exact Agent/Pane/runtime/relaunch-generation/thread tuple", mutationCodexHandoverRelaunch, "all target snapshots, endpoint CAS, app-owned server generation, and exact live mirrored identity", "same Pane runs successor attachment and carries operation/generation receipt"),
