...
func NewWithLifecycleDiagnostics(recorder *diagnostics.LifecycleRecorder) *App {
	...
	// `create` spellings and the provider direct actions reach, so a pane opened
...
func shouldRunLegacyHookMigrations(args []string) bool {
	...
	case "ai":
		return false
	case "agent":
		if len(args) >= 2 && (args[1] == "capabilities" || args[1] == "models" || args[1] == "message" || args[1] == "question" || args[1] == "sessions" ||
			(args[1] == "approval" && len(args) >= 3 && (args[2] == "list" || args[2] == "answer")) || (args[1] == "resume" && hasClaudeDialogueModeFlag(args[2:]))) {
			return false
		}
	case "create":
	...
	case "internal":
		...
		if len(args) >= 2 && (args[1] == "install-residue" || args[1] == "claude-message-wait" || args[1] == "claude-message-reply" || args[1] == "claude-message-boundary" || args[1] == agentMessageReleaseRoute || args[1] == "claude-reply-tool" || args[1] == claudeQuestionHookRoute || args[1] == claudePermissionHookRoute || args[1] == claudeQuestionPickerRoute || args[1] == "claude-dialogue-exec" || args[1] == "claude-dialogue-observe") {
			return false
		}
