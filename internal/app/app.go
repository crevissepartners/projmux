...
func shouldRunLegacyHookMigrations(args []string) bool {
	...
	case "internal":
		if len(args) >= 2 && (args[1] == "codex-generation-launch" || args[1] == "install-residue" || args[1] == "claude-message-wait" || args[1] == "claude-message-reply" || args[1] == "claude-message-boundary" || args[1] == "claude-reply-tool") {
			return false
		}
