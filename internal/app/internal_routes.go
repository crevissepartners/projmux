...
// behavior, so stdout, stderr, the exit code, and the side effects are
// identical to the current spelling. The current spellings survive as hidden
// compatibility routes and are not removed here: a tmux server that is already
// running holds config generated by a previously installed binary and keeps
// invoking them. Removing them is a separate breaking-change Phase.
