...
func (c *aiCommand) runAgentPickerSelection(direction string) error {
	...
	// Every action hands its intent to launchPickerSelection, which closes a
	// popup-hosted picker at once and runs the create after it.
...
func (c *aiCommand) createPaneFromIntent(intent agentPaneIntent) error {
	// An answer-mode picker asks for a Window that does not exist yet, so it
	// never creates: a selection that reaches the funnel is recorded for the
	// producer instead of being committed into the Window the key was pressed
	// in (window_create_launch_choice.go).
...
func (c *aiCommand) requireAIAgentEnabled(mode string, path aiSplitLaunchPath) error {
	message, disabled := c.aiAgentDisabledLaunchMessage(mode, path)
	if !disabled {
		return nil
	}
...
// aiAgentDisabledLaunchMessage is the Settings gate itself, without the ambient
// message requireAIAgentEnabled shows. A producer that knows the exact client
// it acts for reads the gate here and puts the same sentence on that client
// instead, so one disabled provider is still reported exactly once.
func (c *aiCommand) aiAgentDisabledLaunchMessage(mode string, path aiSplitLaunchPath) (string, bool) {
...
// only "visible" when it is the active pane of some attached client's current
// window, which list-clients reports as #{pane_id} (visibleClientPaneFormat).
...
func (c *aiCommand) paneVisibleToClient(paneID string) bool {
	output, err := c.read("tmux", "list-clients", "-F", visibleClientPaneFormat)
	if err != nil {
		return false
	}
...
// nodeManagerCandidates lists the per-version bin paths that common Node
// version managers install an npm-distributed Agent CLI into.
func nodeManagerCandidates(home, binName string) []string {
	...
	return append(candidates, filepath.Join(home, ".volta", "bin", binName))
}
