...
chmod 0700 "$dialogue_root/runtime"
...
if [[ "$dialogue_provider_writes_after" != "0" || "$dialogue_provider_writes_after" != "$dialogue_provider_writes_before" ]]; then
...
PY
dialogue_cleanup
dialogue_processes_gone() { ! ps -eo args= | grep -F "$dialogue_root" | grep -v grep >/dev/null; }
